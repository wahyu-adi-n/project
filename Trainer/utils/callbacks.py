...
class CustomCallback:
    ...
    def __call__(self, model, optimizer, metrics):
        ...
        if self.best_score is None:
            ...
            if self.mode == 'min':
                ...
                NotImplementedError(
                    'Unknown Mode Please Choose Between min/max')
    ...
    def save_checkpoint(self, score, model, optimizer):
        ...
        if self.verbose:
            self.trace_func(
                f'Val {self.metric} got best scored ({self.metric_min:.6f} --> {score:.6f}). Saving Model..')
        if not os.path.exists(self.checkpoint_path):
            os.makedirs(self.checkpoint_path)
        torch.save(model.state_dict(), os.path.join(
            self.checkpoint_path, 'best_model.pth'))
        torch.save(optimizer.state_dict(), os.path.join(
            self.checkpoint_path, 'best_optimizer.pth'))
