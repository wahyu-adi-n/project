...
class Trainer(BaseTrainer):
    ...
    def load_model(self):
        saved_name = os.path.join(self.cfg['output_dir'], '{}_{}.pth'.format(
            self.cfg['model']['base'], self.cfg['dataset']['name']))
    ...
    def save_last_model(self, state: str):
        ...
        model_saved_name = os.path.join(
            self.cfg['output_dir'], f'{state}_model.pth')
        optimizer_saved_name = os.path.join(
            self.cfg['output_dir'], f'{state}_optimizer.pth')
    ...
    def train_one_epoch(self, epoch):
        self.network.train()
        loss = 0.0
        for i, (imgs, labels) in tqdm(enumerate(self.trainloader)):
    ...
    def train(self):
        ...
        for epoch in range(self.cfg['train']['num_epochs']):
            ...
            val_acc, val_prec, val_rec, val_f1, val_loss = self.validate_one_epoch(
                epoch)
        ...
        # for save model after all model has been trained
        self.save_last_model(state="final")
    ...
    def validate_one_epoch(self, epoch):
        ...
        with torch.no_grad():
            for i, (imgs, labels) in enumerate(self.valloader):
                ...
                classification_metrics = self.eval_metrics(
            ...
            if self.logger is not None:
                self.logger.log_metric(
                    "epoch_val_loss", epoch_val_loss, epoch=epoch)
                self.logger.log_metric(
                    "epoch_accuracy", epoch_accuracy, epoch=epoch)
                self.logger.log_metric(
                    "epoch_precision", epoch_precision, epoch=epoch)
                self.logger.log_metric(
                    "epoch_recall", epoch_recall, epoch=epoch)
                self.logger.log_metric(
                    "epoch_f1_score", epoch_f1_score, epoch=epoch)
