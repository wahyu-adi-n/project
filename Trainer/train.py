...
if __name__ == "__main__":
    ...
    parser.add_argument('-cfg', '--config', default="../Trainer/configs/effnetb0.yaml",type=str,
                        help="Path to config yaml file")
    args = parser.parse_args()
    cfg = read_cfg(cfg_file=args.config)
    hyperparameters = generate_hyperparameters(cfg)
    # create logger based on model name for Track each proses in console
    LOG = get_logger(cfg['model']['base'])
    ...
    kwargs = dict(pretrained=cfg['model']['pretrained'],
    ...
    weight = count_weighted(os.path.join(
        cfg.dataset.root_dir, cfg.dataset.train_csv))
    ...
    trainer = Trainer(cfg, network, optimizer, criterion, dataset, device,
                      callbacks=custom_cb, lr_scheduler=lr_scheduler, logger=logger)
    ...
    final_optimizer_path = os.path.join(
        cfg['output_dir'], 'final_optimizer.pth')
